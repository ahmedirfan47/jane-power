...
export interface SymbolMeta {
  ...
  binance?: string;
  mt5?: boolean;
  provider?: boolean; // fed by Twelve Data
}

export const SYMBOLS: SymbolMeta[] = [
  { symbol: "EURUSD", name: "Euro / US Dollar", group: "Forex", base: 1.0842, decimals: 5, vol: 0.0006, mt5: true, provider: true },
  { symbol: "GBPUSD", name: "Pound / US Dollar", group: "Forex", base: 1.2718, decimals: 5, vol: 0.0006, mt5: true, provider: true },
  { symbol: "USDJPY", name: "US Dollar / Yen", group: "Forex", base: 156.32, decimals: 3, vol: 0.0006, mt5: true, provider: true },
  { symbol: "AUDUSD", name: "Aussie / US Dollar", group: "Forex", base: 0.6621, decimals: 5, vol: 0.0007, mt5: true, provider: true },
  { symbol: "USDCAD", name: "US Dollar / Loonie", group: "Forex", base: 1.3662, decimals: 5, vol: 0.0005, mt5: true, provider: true },

  { symbol: "XAUUSD", name: "Gold / US Dollar", group: "Metals", base: 2684.5, decimals: 2, vol: 0.0008, mt5: true, provider: true },
  { symbol: "XAGUSD", name: "Silver / US Dollar", group: "Metals", base: 31.24, decimals: 3, vol: 0.0013, mt5: true, provider: true },
...
export function isLiveSymbol(symbol: string): boolean {
  const m = META[symbol];
  return !!(m?.binance || m?.mt5 || m?.provider);
}